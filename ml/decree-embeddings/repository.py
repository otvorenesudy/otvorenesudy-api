    ...
    while True:
        ...
        cur.execute(
            f"""
              ...
              GROUP BY decrees.id, decree_forms.value
              ORDER BY decrees.id, decree_forms.value
              LIMIT %s
            """,
            (last_id, batch_size),
        )
...
def training_decrees(count=1000):
    cur = db.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
    ...
    logger.info(f"Fetched [{len(rows)}] training decrees in [{time_in_ms:.2f}ms]")
