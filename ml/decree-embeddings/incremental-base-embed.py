...
from repository import (
    decrees,
    decrees_base_features_vocabulary,
...
if __name__ == "__main__":
    try:
        vocabulary = decrees_base_features_vocabulary()
        scaler = StandardScaler()
        pca = IncrementalPCA(n_components=768)

        for batch in decrees(include_text=False):
            embeddings = base_embed_decrees(vocabulary, batch)
