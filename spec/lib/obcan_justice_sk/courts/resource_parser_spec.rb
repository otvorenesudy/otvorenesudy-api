...
RSpec.describe ObcanJusticeSk::Courts::ResourceParser do
  let(:uri) { 'https://obcan.justice.sk/infosud/-/infosud/reg-detail/sud/sud_102' }
  let(:html) { ObcanJusticeSk::Downloader.download(uri) }

  describe '.parse' do
    it 'parses raw court attributes', vcr: { cassette_name: 'obcan_justice_sk/court' } do
      attributes = ObcanJusticeSk::Courts::ResourceParser.parse(html)
    ...
    context 'when court has more vice-chairmen' do
      ...
      it 'parses vice-chairmen correctly', vcr: { cassette_name: 'obcan_justice_sk/court_with_more_vice_chairmen' } do
        attributes = ObcanJusticeSk::Courts::ResourceParser.parse(html)
    ...
    context 'when court has no contact' do
      ...
      it 'does not parse contact', vcr: { cassette_name: 'obcan_justice_sk/court_without_contact' } do
        attributes = ObcanJusticeSk::Courts::ResourceParser.parse(html)
    ...
    context 'when court is missing media phone number' do
      ...
      it 'parses court correctly', vcr: { cassette_name: 'obcan_justice_sk/court_with_no_media_phone_number' } do
        attributes = ObcanJusticeSk::Courts::ResourceParser.parse(html)
