...
RSpec.describe InfoSud::Importer do
  describe '.import' do
    ...
    it 'imports file from path' do
      ...
      expect(repository).to receive(:import_from).with(guid: '1', data: { guid: '1', attribute: 'A' })
      expect(repository).to receive(:import_from).with(guid: '2', data: { guid: '2', attribute: 'B' })
